    ...
    def _generate_graph(self, data_dict):
        if self.otf_graph:
            ...
            if self.always_use_pbc:
                # Mo: this we changed to zeros for omol
                pbc = torch.zeros(len(data_dict), 3, dtype=torch.bool)
            else:
                assert (
                    "pbc" in data_dict
            ...
            if not torch.all(valid_edges):
                ...
                if "edge_index" in data_dict:
                    data_dict["edge_index"] = data_dict["edge_index"][:, valid_edges]
...
class eSEN(nn.Module):
    ...
    def __init__(
        ...
        direct_forces: bool = True,  # Only predicting energy
        regress_forces: bool = False,  # Only predicting energy
        regress_stress: bool = False,  # Only predicting energy
        activation_checkpointing: bool = False,
        chg_spin_emb_type: str = "pos_emb",
    ...
    def forward(self, data_dict):
        ...
        Args:
            data_dict: Dictionary containing input data with keys:
                ...
                - charge: system charges [batch_size]
                - spin: system spins [batch_size]
